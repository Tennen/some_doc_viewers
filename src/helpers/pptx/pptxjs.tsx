...
//var worker;
const parsePPTX = async (options: any) => {
    ...
    let _order = 1;
    let app_verssion;
    let rtl_langs_array = ["he-IL", "ar-AE", "ar-SA", "dv-MV", "fa-IR", "ur-PK"]
    let slideFactor = 96 / 914400;
    let fontSizeFactor = 4 / 3.2;
    ////////////////////// 
    let slideWidth = 0;
    let slideHeight = 0;
    ...
    const result = (
        <div>
            <div className="slides-loadnig-msg" style={{ display: "block", width: "100%", color: "white", backgroundColor: "#ddd" }}>
                <div className="slides-loading-progress-bar" style={{ width: percent + "%", backgroundColor: "#4775d1" }}>
                    <span style={{ textAlign: "center" }}>Loading...(" + percent + "%)</span>
                </div>
            </div>
        </div>
    )
    if (settings.slideMode) {
        // TODO: implement divs2slides
    }
    ...
    if (settings.url != "") {
        try {
            ...
            const file = await JSZip.loadAsync(arrayBuffer);
            convertToHtml(file);
        } catch (e) {
        }
    }
    if (settings.file != "") {
        try {
            const file = await JSZip.loadAsync(settings.file);
            convertToHtml(file);
        } catch (e) {
        }
    }
    ...
    function updateProgressBar(percent: number) {
        let progressBarElemtnt = $(".slides-loading-progress-bar")
    ...
    function convertToHtml(zip: JSZip) {
        let rslt_ary = processPPTX(zip);
        ...
        let sScale = settings.slidesScale;
        let trnsfrmScl = "";
        if (sScale != "") {
            let numsScale = parseInt(sScale);
            let scaleVal = numsScale / 100;
            if (settings.slideMode && settings.slideType != "revealjs") {
                trnsfrmScl = 'transform:scale(' + scaleVal + '); transform-origin:top';
            }
        }

        let slidesHeight = $("#" + divId + " .slide").height();
        let numOfSlides = $("#" + divId + " .slide").length;
        let sScaleVal = (sScale != "") ? scaleVal : 1;
    ...
    function initSlideMode(divId, settings) {
        ...
        if (settings.slideType == "" || settings.slideType == "divs2slidesjs") {
            ...
            $("#" + divId + " .slide").hide();
            setTimeout(function () {
                let slideConf = settings.slideModeConfig;
                $(".slides-loadnig-msg").remove();
                $("#" + divId).divs2slides({
    ...
    function processPPTX(zip) {
        ...
        if (zip.file("docProps/thumbnail.jpeg") !== null) {
            let pptxThumbImg = base64ArrayBuffer(zip.file("docProps/thumbnail.jpeg").asArrayBuffer());
            post_ary.push({
                "type": "pptx-thumb",
                "data": pptxThumbImg,
                "slide_num": -1
        ...
        let filesInfo = getContentTypes(zip);
        let slideSize = getSlideSizeAndSetDefaultTextStyle(zip);
        ...
        let numOfSlides = filesInfo["slides"].length;
        for (var i = 0; i < numOfSlides; i++) {
            let filename = filesInfo["slides"][i];
            ...
            let slideHtml = processSingleSlide(zip, filename, i, slideSize);
            post_ary.push({
                "type": "slide",
                "data": slideHtml,
    ...
    function readXmlFile(zip: JSZip, filename: string, isSlideContent: boolean) {
        try {
    ...
    function getContentTypes(zip) {
        let ContentTypesJson = readXmlFile(zip, "[Content_Types].xml");
    ...
    function getSlideSizeAndSetDefaultTextStyle(zip) {
        //get app version
        let app = readXmlFile(zip, "docProps/app.xml");
        let app_verssion_str = app["Properties"]["AppVersion"]
        app_verssion = parseInt(app_verssion_str);

        //get slide dimensions
        let rtenObj = {};
        let content = readXmlFile(zip, "ppt/presentation.xml");
        ...
        //Scale
        // let viewProps = readXmlFile(zip, "ppt/viewProps.xml");
        // let scaleLoc = getTextByPathList(viewProps, ["p:viewPr", "p:slideViewPr", "p:cSldViewPr", "p:cViewPr","p:scale"]);
        // let scaleXnodes, scaleX = 1, scaleYnode, scaleY = 1;
        // if (scaleLoc !== undefined){
        //     scaleXnodes = scaleLoc["a:sx"]["attrs"];
        //     let scaleXnodesN = scaleXnodes["n"];
        //     let scaleXnodesD = scaleXnodes["d"];
        ...
        //     let scaleYnodeN = scaleYnode["n"];
        //     let scaleYnodeD = scaleYnode["d"];
    ...
    function processSingleSlide(zip, sldFileName, index, slideSize) {
        ...
        */
        ...
        let resContent = readXmlFile(zip, resName);
        ...
        let slideLayoutContent = readXmlFile(zip, layoutFilename);
        let slideLayoutTables = indexNodes(slideLayoutContent);
        ...
        let slideLayoutResContent = readXmlFile(zip, slideLayoutResFilename);
        ...
        let slideMasterContent = readXmlFile(zip, masterFilename);
        let slideMasterTextStyles = getTextByPathList(slideMasterContent, ["p:sldMaster", "p:txStyles"]);
        let slideMasterTables = indexNodes(slideMasterContent);
        ...
        let slideMasterResContent = readXmlFile(zip, slideMasterResFilename);
        ...
        if (themeFilename !== undefined) {
            ...
            let themeContent = readXmlFile(zip, themeFilename);
            let themeResContent = readXmlFile(zip, themeResFileName);
        ...
        if (diagramFilename !== undefined) {
            ...
            let digramResContent = readXmlFile(zip, diagramResFileName);
        ...
        // =====< Step 3 >=====
        let slideContent = readXmlFile(zip, sldFileName, true);
        ...
        if (settings.slideMode && settings.slideType == "revealjs") {
            ...
            let result = "<div class='slide' style='width:" + slideSize.width + "px; height:" + slideSize.height + "px;" + bgColor + "'>"
        }
        result += bgResult;
    ...
    function processGroupSpNode(node, warpObj, source) {
        ...
        if (xfrmNode !== undefined) {
            let x = parseInt(xfrmNode["a:off"]["attrs"]["x"]) * slideFactor;
            let y = parseInt(xfrmNode["a:off"]["attrs"]["y"]) * slideFactor;
            let chx = parseInt(xfrmNode["a:chOff"]["attrs"]["x"]) * slideFactor;
            let chy = parseInt(xfrmNode["a:chOff"]["attrs"]["y"]) * slideFactor;
            let cx = parseInt(xfrmNode["a:ext"]["attrs"]["cx"]) * slideFactor;
            let cy = parseInt(xfrmNode["a:ext"]["attrs"]["cy"]) * slideFactor;
            let chcx = parseInt(xfrmNode["a:chExt"]["attrs"]["cx"]) * slideFactor;
            let chcy = parseInt(xfrmNode["a:chExt"]["attrs"]["cy"]) * slideFactor;
    ...
    function processSpNode(node, pNode, warpObj, source, sType) {
        ...
        */
        ...
        let order = getTextByPathList(node, ["attrs", "order"]);
        let isUserDrawnBg;
    ...
    function genShape(node, pNode, slideLayoutSpNode, slideMasterSpNode, id, name, idx, type, order, warpObj, isUserDrawnBg, sType, source) {
        ...
        if (shapType !== undefined || custShapType !== undefined /*&& slideXfrmNode !== undefined*/) {
            let off = getTextByPathList(slideXfrmNode, ["a:off", "attrs"]);
            let x = parseInt(off["x"]) * slideFactor;
            let y = parseInt(off["y"]) * slideFactor;

            let ext = getTextByPathList(slideXfrmNode, ["a:ext", "attrs"]);
            let w = parseInt(ext["cx"]) * slideFactor;
            let h = parseInt(ext["cy"]) * slideFactor;

            let svgCssName = "_svg_css_" + (Object.keys(styleTable).length + 1) + "_" + Math.floor(Math.random() * 1001);
            ...
            if (clrFillType == "GRADIENT_FILL") {
                ...
                imgFillFlg = true;
                let svgBgImg = getSvgImagePattern(node, fillColor, shpId, warpObj);
                //fill="url(#imgPtrn)"
            ...
            // Border Color
            let border = getBorder(node, pNode, true, "shape", warpObj);

            let headEndNodeAttrs = getTextByPathList(node, ["p:spPr", "a:ln", "a:headEnd", "attrs"]);
            let tailEndNodeAttrs = getTextByPathList(node, ["p:spPr", "a:ln", "a:tailEnd", "attrs"]);
            ...
            if (outerShdwNode !== undefined) {
                let chdwClrNode = getSolidFill(outerShdwNode, undefined, undefined, warpObj);
                ...
                let dist = parseInt(outerShdwAttrs["dist"]) * slideFactor;//(px) //* (3 / 4); //(pt)
                //var rotWithShape = outerShdwAttrs["rotWithShape"];
                let blurRad = (outerShdwAttrs["blurRad"]) ? (parseInt(outerShdwAttrs["blurRad"]) * slideFactor) : ""; //+ "px"
        ...
        if (shapType !== undefined && custShapType === undefined) {
            ...
            switch (shapType) {
                ...
                case "flowChartPunchedCard":
                    let shapAdjst_ary = getTextByPathList(node, ["p:spPr", "a:prstGeom", "a:avLst", "a:gd"]);
                ...
                case "flowChartMerge":
                    let shapAdjst = getTextByPathList(node, ["p:spPr", "a:prstGeom", "a:avLst", "a:gd", "attrs", "fmla"]);
                ...
                case "star4":
                    ...
                    let cnstVal1 = 50000 * slideFactor;
                    let shapAdjst = getTextByPathList(node, ["p:spPr", "a:prstGeom", "a:avLst", "a:gd"]);//[0]["attrs"]["fmla"];
                ...
                case "pieWedge":
                case "arc":
                    let shapAdjst = getTextByPathList(node, ["p:spPr", "a:prstGeom", "a:avLst", "a:gd"]);
                ...
                case "noSmoking":
                    ...
                    let ct1, st1, m1, n1, dx1, dy1, x1, y1, y1, y2;
                ...
                case "leftUpArrow":
                    ...
                    let vc = h / 2, hc = w / 2, a1, a2, a3, x1, x2, dx4, dx3, x3, x4, x5, y2, y3, y4, y5, maxAdj1, maxAdj3;
                    let minWH = Math.min(w, h);
                    if (adj2 < 0) a2 = 0
                    else if (adj2 > cnstVal1) a2 = cnstVal1
                    else a2 = adj2
                ...
                case "bentUpArrow":
                    ...
                    let vc = h / 2, hc = w / 2, a1, a2, a3, dx1, x1, dx2, x2, dx3, x3, x4, y1, y2, dy2;
                    let minWH = Math.min(w, h);
                    if (adj1 < 0) a1 = 0
                    else if (adj1 > cnstVal1) a1 = cnstVal1
                    else a1 = adj1
                ...
                case "bentArrow":
                    ...
                    let th, aw2, th2, dh2, ah, bw, bh, bs, bd, bd3, bd2,
                ...
                case "uturnArrow":
                    ...
                    let th, aw2, th2, dh2, ah, bw, bs, bd, bd3, bd2,
                        th = minWH * a1 / cnstVal2;
                    aw2 = minWH * a2 / cnstVal2;
                    th2 = th / 2;
                    dh2 = aw2 - th2;
                ...
                case "curvedUpArrow":
                    ...
                    let stAng2dg, swAng2dg, swAngDg, swAng2dg;
            ...
            let maxX = parseInt(pathNodes["attrs"]["w"]);// * slideFactor;
            let maxY = parseInt(pathNodes["attrs"]["h"]);// * slideFactor;
            let cX = (1 / maxX) * w;
            let cY = (1 / maxY) * h;
            //console.log("w = "+w+"\nh = "+h+"\nmaxX = "+maxX +"\nmaxY = " + maxY);
                ...
                Object.keys(moveToNode).forEach(function (key) {
                    ...
                    if (moveToPtNode !== undefined) {
                        Object.keys(moveToPtNode).forEach(function (key2) {
                            ...
                            let spX = moveToNoPt["attrs", "x"];//parseInt(moveToNoPt["attrs", "x"]) * slideFactor;
                            let spY = moveToNoPt["attrs", "y"];//parseInt(moveToNoPt["attrs", "y"]) * slideFactor;
    ...
    */
    function processPicNode(node, warpObj, source, sType) {
        ...
        let zip = warpObj["zip"];
        let imgArrayBuffer = zip.file(imgName).asArrayBuffer();
        let mimeType = "";
        ...
        let mediaProcess = settings.mediaProcess;
    ...
    function genTextBody(textBodyNode, spNode, slideLayoutSpNode, slideMasterSpNode, type, idx, warpObj, tbl_col_width) {
        ...
        for (var i = 0; i < apNode.length; i++) {
            ...
            //rtlStr = "";//"dir='"+isRTL+"'";
            let styleText = "";
            let marginsVer = getVerticalMargins(pNode, textBodyNode, type, idx, warpObj);
            if (marginsVer != "") {
            ...
            let sld_prg_width = ((prg_width_node !== undefined) ? ("width:" + (parseInt(prg_width_node) * slideFactor) + "px;") : "width:inherit;");
            let sld_prg_height = ((prg_height_node !== undefined) ? ("height:" + (parseInt(prg_height_node) * slideFactor) + "px;") : "");
            let prg_dir = getPregraphDir(pNode, textBodyNode, idx, type, warpObj);
            ...
            //get text margin 
            let margin_ary = getPregraphMargn(pNode, idx, type, isBullate, warpObj);
    ...
    function genBuChar(node, i, spNode, textBodyNode, pFontStyle, idx, type, warpObj) {
        ...
        if (marLNode !== undefined) {
            let marginLeft = parseInt(marLNode) * slideFactor;
        ...
        if (marRNode !== undefined) {
            let marginRight = parseInt(marRNode) * slideFactor;
            if (isRTL) {// && alignNode == "r") {
                marLStr = "padding-right:";// "margin-right: ";
            } else {
                marLStr = "padding-left:";//"margin-left: ";
        ...
        if (buType == "TYPE_BULLET") {
            ...
            // let marginLeft = parseInt(getTextByPathList(marLNode)) * slideFactor;
            // let marginRight = parseInt(getTextByPathList(marRNode)) * slideFactor;
            // if (isNaN(marginLeft)) {
            ...
            // let marginLeft = parseInt(getTextByPathList(pPrNode, ["attrs", "marL"])) * slideFactor;
            ...
            // let marginLeft = parseInt(getTextByPathList(pPrNode, ["attrs", "marL"])) * slideFactor;
            // let marginRight = parseInt(getTextByPathList(pPrNode, ["attrs", "marR"])) * slideFactor;
            ...
            if (buPicId !== undefined) {
                ...
                let imgMimeType = getMimeType(imgExt);
    ...
    function genSpanElement(node, rIndex, pNode, textBodyNode, pFontStyle, slideLayoutSpNode, idx, type, rNodeLength, warpObj, isBullate) {
        ...
        if (linkID !== undefined) {
            ...
            let rPrlinkClr = getSolidFill(linkClrNode, undefined, undefined, warpObj);
        ...
        /////////////////////////////////////////////////////////////////////////////////////
        //getFontColor
        let fontClrPr = getFontColorPr(node, pNode, lstStyle, pFontStyle, lvl, idx, type, warpObj);
        ...
        let font_size = getFontSize(node, textBodyNode, pFontStyle, lvl, type, warpObj);
    ...
    function genTable(node, warpObj) {
        ...
        if (tbleStyleId !== undefined) {
            let tbleStylList = tableStyles["a:tblStyleLst"]["a:tblStyle"];
        ...
        for (var i = 0; i < trNodes.length; i++) {
            ...
            if (thisTblStyle !== undefined && thisTblStyle["a:wholeTbl"] !== undefined) {
                ...
                if (bgFillschemeClr !== undefined) {
                    let local_fillColor = getSolidFill(bgFillschemeClr, undefined, undefined, warpObj);
                ...
                if (rowTxtStyl !== undefined) {
                    let local_fontColor = getSolidFill(rowTxtStyl, undefined, undefined, warpObj);
            ...
            if (i == 0 && tblStylAttrObj["isFrstRowAttr"] == 1 && thisTblStyle !== undefined) {
                ...
                if (borderStyl !== undefined) {
                    let local_row_borders = getTableBorders(borderStyl, warpObj);
                ...
                if (rowTxtStyl !== undefined) {
                    let local_fontClrPr = getSolidFill(rowTxtStyl, undefined, undefined, warpObj);
    ...
    function getTableCellParams(tcNodes, getColsGrid, row_idx, col_idx, thisTblStyle, cellSource, warpObj) {
        ...
        let text = genTextBody(tcNodes["a:txBody"], tcNodes, undefined, undefined, undefined, undefined, warpObj, total_col_width);//tableStyles
        ...
        if (lin_bottm !== undefined && lin_bottm != "") {
            let bottom_line_border = getBorder(lin_bottm, undefined, false, "", warpObj)
            if (bottom_line_border != "") {
                colStyl += "border-bottom:" + bottom_line_border + ";";
            }
        }
        if (lin_top !== undefined && lin_top != "") {
            let top_line_border = getBorder(lin_top, undefined, false, "", warpObj);
            if (top_line_border != "") {
                colStyl += "border-top: " + top_line_border + ";";
            }
        }
        if (lin_left !== undefined && lin_left != "") {
            let left_line_border = getBorder(lin_left, undefined, false, "", warpObj)
            if (left_line_border != "") {
                colStyl += "border-left: " + left_line_border + ";";
            }
        }
        if (lin_right !== undefined && lin_right != "") {
            let right_line_border = getBorder(lin_right, undefined, false, "", warpObj)
        ...
        // if (borderStyl !== undefined) {
        //     let local_col_borders = getTableBorders(borderStyl, warpObj);
    ...
    function genChart(node, warpObj) {
        ...
        let content = readXmlFile(warpObj["zip"], refName);
    ...
    function genDiagram(node, warpObj, source, sType) {
        ...
        let dgmClr = readXmlFile(zip, dgmClrFileName);
        let dgmData = readXmlFile(zip, dgmDataFileName);
        let dgmLayout = readXmlFile(zip, dgmLayoutFileName);
        let dgmQuickStyle = readXmlFile(zip, dgmQuickStyleFileName);
        ...
        let dgmDrwSpArray = getTextByPathList(warpObj["digramFileContent"], ["p:drawing", "p:spTree", "p:sp"]);
        let rslt = "";
    ...
    function getPosition(slideSpNode, pNode, slideLayoutSpNode, slideMasterSpNode, sType) {
        ...
        if (sType == "group") {
            ...
            let grpXfrmNode = getTextByPathList(pNode, ["p:grpSpPr", "a:xfrm"]);
            if (xfrmNode !== undefined) {
                grpX = parseInt(grpXfrmNode["a:off"]["attrs"]["x"]) * slideFactor;
                grpY = parseInt(grpXfrmNode["a:off"]["attrs"]["y"]) * slideFactor;
                // let chx = parseInt(grpXfrmNode["a:chOff"]["attrs"]["x"]) * slideFactor;
                // let chy = parseInt(grpXfrmNode["a:chOff"]["attrs"]["y"]) * slideFactor;
                // let cx = parseInt(grpXfrmNode["a:ext"]["attrs"]["cx"]) * slideFactor;
                // let cy = parseInt(grpXfrmNode["a:ext"]["attrs"]["cy"]) * slideFactor;
                // let chcx = parseInt(grpXfrmNode["a:chExt"]["attrs"]["cx"]) * slideFactor;
                // let chcy = parseInt(grpXfrmNode["a:chExt"]["attrs"]["cy"]) * slideFactor;
        ...
        if (sType == "group-rotate" && pNode["p:grpSpPr"] !== undefined) {
            ...
            // let ox = parseInt(xfrmNode["a:off"]["attrs"]["x"]) * slideFactor;
            // let oy = parseInt(xfrmNode["a:off"]["attrs"]["y"]) * slideFactor;
            let chx = parseInt(xfrmNode["a:chOff"]["attrs"]["x"]) * slideFactor;
            let chy = parseInt(xfrmNode["a:chOff"]["attrs"]["y"]) * slideFactor;
    ...
    function getVerticalMargins(pNode, textBodyNode, type, idx, warpObj) {
        ...
        if (isInLayoutOrMaster && (spcBefNode === undefined || spcAftNode === undefined || lnSpcNode === undefined)) {
            //check in layout
            if (idx !== undefined) {
                let laypPrNode = getTextByPathList(warpObj, ["slideLayoutTables", "idxTable", idx, "p:txBody", "a:p", (lvl - 1), "a:pPr"]);
    ...
    function getContentDir(node, type, warpObj) {
        ...
        // else {
        //     if (type == "textBox") {
        //         let dirVal = getTextByPathList(warpObj, ["defaultTextStyle", "a:lvl1pPr", "attrs", "rtl"]);
        //         if (dirVal == "1") {
        //             return "content-rtl";
        //         }
        //     }
    ...
    function getFontType(node, type, warpObj, pFontStyle) {
        ...
        if (typeface === undefined) {
            ...
            let fontSchemeNode = getTextByPathList(warpObj["themeContent"], ["a:theme", "a:themeElements", "a:fontScheme"]);
            if (fontIdx == "") {
                if (type == "title" || type == "subTitle" || type == "ctrTitle") {
                    fontIdx = "major";
                } else {
    ...
    function getFontColorPr(node, pNode, lstStyle, pFontStyle, lvl, idx, type, warpObj) {
        ...
        if (txtGlowNode !== undefined) {
            ...
            let rad = (txtGlowNode["attrs"]["rad"]) ? (txtGlowNode["attrs"]["rad"] * slideFactor) : 0;
    ...
    function getBorder(node, pNode, isSvgMode, bType, warpObj) {
        ...
        if (lineNode !== undefined) {
            ...
            // Border type
            let borderType = getTextByPathList(lineNode, ["a:prstDash", "attrs", "val"]);
    ...
    function getBackground(warpObj, slideSize, index) {
        ...
        let bgColor = getSlideBackgroundFill(warpObj, index);
    ...
    function getBgPicFill(bgPr, sorce, warpObj, phClr, index) {
        ...
        if (stretch !== undefined) {
            let fillRect = getTextByPathList(stretch, ["a:fillRect", "attrs"]);
            //console.log("getBgPicFill=>bgPr: ", bgPr)
    ...
    function getSchemeColorFromTheme(schemeClr, clrMap, phClr, warpObj) {
        ...
        if (clrMap !== undefined) {
            ...
            if (sldClrMapOvr !== undefined) {
                ...
                let sldClrMapOvr = getTextByPathList(warpObj["slideLayoutContent"], ["p:sldLayout", "p:clrMapOvr", "a:overrideClrMapping", "attrs"]);
                if (sldClrMapOvr !== undefined) {
                    slideLayoutClrOvride = sldClrMapOvr;
                } else {
        ...
        if (schmClrName == "phClr" && phClr !== undefined) {
            ...
            let refNode = getTextByPathList(warpObj["themeContent"], ["a:theme", "a:themeElements", "a:clrScheme", schemeClr]);
    ...
    function getSvgImagePattern(node, fill, shpId, warpObj) {
        ...
        if (tileNode !== undefined && tileNode["sx"] !== undefined) {
            let sx = (parseInt(tileNode["sx"]) / 100000) * width;
            let sy = (parseInt(tileNode["sy"]) / 100000) * height;
        }
        ...
        if (sx !== undefined && sx != 0) {
            let ptrn = '<pattern id="imgPtrn_' + shpId + '" x="0" y="0"  width="' + sx + '" height="' + sy + '" patternUnits="userSpaceOnUse">';
        } else {
    ...
    function archaicNumbers(arr) {
        let arrParse = arr.slice().sort(function (a, b) { return b[1].length - a[1].length });
        return {
            format: function (n) {
                let ret = '';
